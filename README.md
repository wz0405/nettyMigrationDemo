...
본 프로젝트는 실제 운영 중인 선불결제 시스템의 서버 레이어 구조를 기반으로 작성한 샘플입니다. 비즈니스 로직(잔액 처리, 한도 검증, 
부분환불 등)은 회사 소유 코드로 포함하지 않으며, 핵심 설계 의도인 Netty 이벤트 루프 기반 API 처리 구조와 Strategy + Factory 패턴에 집중합니다.
